Shader "Unlit/DirectionalFlowShader"
{
    Properties
    {
        [Header(Textures)]
        ...
        _WaterHeightTexture ("Water Height Texture", 2D) = "white" {}
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            TEXTURE2D(_WaterHeightTexture);         SAMPLER(sampler_WaterHeightTexture);
            ...
            float4 frag(Varyings input) : SV_Target
            {
                ...
                FlowCell(_FlowMap, sampler_FlowMap, _NormalMap, sampler_NormalMap, _DisplacementMap, sampler_DisplacementMap, input.uv, _GridResolution_A, _FlowVelocityStrength_A, _WavePeriod_A, curNormal, curDisplacement);
                finalNormal = curNormal;
                finalDisplacement = smoothstep(_HeightEdge_A, 1.0, curDisplacement);

                FlowCell(_FlowMap, sampler_FlowMap, _NormalMap, sampler_NormalMap, _DisplacementMap, sampler_DisplacementMap, input.uv, _GridResolution_B, _FlowVelocityStrength_B, _WavePeriod_B, curNormal, curDisplacement);
                finalNormal += curNormal;
                finalDisplacement += smoothstep(_HeightEdge_B, 1.0, curDisplacement);

                FlowCell(_FlowMap, sampler_FlowMap, _NormalMap, sampler_NormalMap, _DisplacementMap, sampler_DisplacementMap, input.uv, _GridResolution_C, _FlowVelocityStrength_C, _WavePeriod_C, curNormal, curDisplacement);
                finalNormal += curNormal;
                finalDisplacement += smoothstep(_HeightEdge_C, 1.0, curDisplacement);

                FlowCell(_FlowMap, sampler_FlowMap, _NormalMap, sampler_NormalMap, _DisplacementMap, sampler_DisplacementMap, input.uv, _GridResolution_D, _FlowVelocityStrength_D, _WavePeriod_D, curNormal, curDisplacement);
                finalNormal += curNormal;
                ...
                // 深浅水颜色
                float waterHeight = SAMPLE_TEXTURE2D(_WaterHeightTexture, sampler_WaterHeightTexture, input.uv);
